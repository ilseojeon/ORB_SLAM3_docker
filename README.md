...
I was able to fix the error when ROS was installed before installing it and somehow dependencies were accomplished.

Original Codes from [https://github.com/UZ-SLAMLab/ORB_SLAM3](https://github.com/UZ-SLAMLab/ORB_SLAM3)
...
Run build_docker.sh file

```bash
chomod +x build_docker.sh
./build_docker.sh
```
...
```bash
docker run --gpus all -it --ipc=host --net=host --privileged -e DISPLAY=unix$DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix:rw -e NVIDIA_DRIVER_CAPABILITIES=all --name orbslam-test orbslam3:latest /bin/bash
```
